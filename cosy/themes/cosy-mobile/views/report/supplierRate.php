...
<div >
	...
	<p><b>Ammount of paid rates by supplier:</b></p>
</div>
					...
					foreach($suppliers as $supplier)
					{
						echo '<tr>';
							...
							$criteria->params=array(
								...
								echo '<td>'.$this->getTotalRatePago($criteria) .'</td>';
						echo '</tr>';
					}	
			echo "</table>";	
			echo '<br />';
		...
		$months = array(array("01","Jan"), array("02","Feb"), array("03","Mar"), 
					...
					$nextMonth = (int)$month[0] + 1;
						...
						echo "['$month[1]' ";				
									foreach($suppliers as $supplier)
									{
										...
										$criteria->params=array(
											':day_start'=> $yearForQuery . '-' . $currentMonth . '-01',
											':day_end'=> $yearForQuery . '-' . $currentMonth . '-01',
											':supplier'=> $supplier->id
											);				
			...
			else
			{
				foreach($months as $month)
				{
					...
					else //Ano e Mês Actual até ao dia actua
					{
						if($currentMonth == $thisMonth)
						{	echo "['$month[1]' ";				
								foreach($suppliers as $supplier)
								{
									...
									$criteria->params=array(
										':day_start'=> $yearForQuery . '-' . $currentMonth . '-01',
										':day_end'=> $yearForQuery . '-' . $nextMonth . '-'.$today,
										':supplier'=> $supplier->id
										);				
							  echo ', ' . $this->getTotalRatePago($criteria);
								}
