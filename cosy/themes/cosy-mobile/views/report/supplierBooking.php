<?php $this->pageTitle=Yii::app()->name; 
	...
	$ourscript = 'function redirect() {
		parent.location = \'index.php?r=report/supplierBooking&year=\' + $(\'#year\').val();
	}';
...
<div >
	...
	<p><b>Number of bookings by supplier:</b></p>
</div>

<div class="div_report_data2" style="padding: 5px; border:solid 0px;">
<div class="div_report_data" style="padding: 5px;border:solid 0px;" >
	<?php 
		$suppliers = Supplier::model()->findAll();
	
	if($yearForQuery == $thisYear)
	{
			//echo '<p style="border: solid 1px;">';
			echo '<table class="table_report_data">';
				...
				echo '<tr>';
						...
						echo '<td>'.$this->getTotalReservas($criteria) .'</td>';
				echo '</tr>';
				}	
			echo "</table>";	
			echo '<br />';
...
<script type="text/javascript">
  ...
  function drawChart() {
	...
	data.addRows([
		...
		if($yearForQuery == $thisYear)
		{
			foreach($months as $month) //Meses Passados do Ano Corrente
			{
				...
				if($currentMonth < $thisMonth)
				{
					echo "['$month[1]'";				
						foreach($suppliers as $supplier)
						{
							...
							echo ', ' . $this->getTotalReservas($criteria);
						}					
					echo "], ";
				}
				else	
				  ...
				  if($currentMonth == $thisMonth)
					{
						...
						foreach($suppliers as $supplier)
						{
							...
							$criteria->params=array(
								':day_start'=> $yearForQuery . '-' . $currentMonth . '-01',
								':day_end'=> $yearForQuery . '-' . $currentMonth . '-' . $today,
								':supplier'=> $supplier->id
								);
  ...
  function drawPieChart() {
		...
		<?php
			...
			if($yearForQuery < $year || $yearForQuery > $year)
			{
				...
				$criteria->condition='day>=:day_start AND day<:day_end AND id_supplier =:supplier';
				$criteria->params=array(
						':day_start'=> $yearToSearch . '-01-01',
						':day_end'=> $yearToSearch . '-12-01',
						':supplier'=> $supplier->id);
				
					echo "data.setValue($cont, 0,'$supplier->name');";
					echo "data.setValue($cont, 1," . $this->getTotalReservas($criteria) . ");";
					$cont++;
				}
			}
			else
			{
				...
				$criteria->params=array(
						':day_start'=> $yearToSearch . '-01-01',
						':day_end'=> $yearToSearch . '-'. $thisMonth . '-' .$today,
						':supplier'=> $supplier->id);
